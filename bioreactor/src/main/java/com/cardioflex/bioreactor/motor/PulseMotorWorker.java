...
public class PulseMotorWorker {
     ...
     * <p>
     * TODO: 后续需要将配置文件加载出来（从数据库），确保重启机器后能继续正常工作
     *
     * @throws IOException
     */
    ...
    public static PulseMotor.PulseMotorConfig getConfig(String tag) {
        PulseMotor motor = motors.get(tag);
        if (motor == null) return new PulseMotor.PulseMotorConfig();
        PulseMotor.PulseMotorConfig config = motor.getConfig();
        if (config == null) return new PulseMotor.PulseMotorConfig();
        return config;
    }
}
