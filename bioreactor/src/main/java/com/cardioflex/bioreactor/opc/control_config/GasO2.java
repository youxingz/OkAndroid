...
public class GasO2 extends ControlConfig<GasO2.GasO2Payload> {
    private static NodeId RESET_CMD = BioreactorNodeId.Equit_O2_DosageReset;
    private static GasO2 instance;

    public static GasO2 getInstance() {
        if (instance == null) {
            instance = new GasO2();
        }
        return instance;
    ...
    private GasO2() {
        ...
        configNodeIdsRead.add(BioreactorNodeId.Equit_O2_ControlMode); // 控制方式0停止1控制循环3手动
        configNodeIdsRead.add(BioreactorNodeId.Equit_O2_Dosage);
        configNodeIdsRead.add(BioreactorNodeId.Equit_O2_PV);
        configNodeIdsRead.add(BioreactorNodeId.Equit_O2_SV);
        configNodeIdsWrite = new ArrayList<>();
        configNodeIdsWrite.add(BioreactorNodeId.Equit_O2_ControlMode); // 控制方式0停止1控制循环3手动
        configNodeIdsWrite.add(BioreactorNodeId.Equit_O2_Dosage);
        configNodeIdsWrite.add(BioreactorNodeId.Equit_O2_PV);
        configNodeIdsWrite.add(BioreactorNodeId.Equit_O2_SV);
    }

    @Override
    public GasO2Payload readConfig() throws OkOPCException {
        try {
            List<DataValue> values = client().read(configNodeIdsRead).get();
            GasO2.GasO2Payload payload = new GasO2.GasO2Payload();
    ...
    @Override
    public void writeConfig(GasO2Payload payload) throws OkOPCException {
        try {
    ...
    public static class GasO2Payload {
        ...
        public GasO2Payload(Boolean isOn, Float pv, Float sv, Float total) {
            this.isOn = isOn;
            this.pv = pv;
            this.sv = sv;
            this.total = total;
