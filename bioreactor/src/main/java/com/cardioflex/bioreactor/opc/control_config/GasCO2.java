...
public class GasCO2 extends ControlConfig<GasCO2.GasCO2Payload> {
    private static NodeId RESET_CMD = BioreactorNodeId.Equit_CO2_DosageReset;
    private static GasCO2 instance;

    public static GasCO2 getInstance() {
        if (instance == null) {
            instance = new GasCO2();
        }
        return instance;
    ...
    private GasCO2() {
        ...
        configNodeIdsRead.add(BioreactorNodeId.Equit_CO2_ControlMode); // 控制方式0停止1控制循环3手动
        configNodeIdsRead.add(BioreactorNodeId.Equit_CO2_Dosage);
        configNodeIdsRead.add(BioreactorNodeId.Equit_CO2_PV);
        configNodeIdsRead.add(BioreactorNodeId.Equit_CO2_SV);
        configNodeIdsWrite = new ArrayList<>();
        configNodeIdsWrite.add(BioreactorNodeId.Equit_CO2_ControlMode); // 控制方式0停止1控制循环3手动
        configNodeIdsWrite.add(BioreactorNodeId.Equit_CO2_Dosage);
        configNodeIdsWrite.add(BioreactorNodeId.Equit_CO2_PV);
        configNodeIdsWrite.add(BioreactorNodeId.Equit_CO2_SV);
    }

    @Override
    public GasCO2Payload readConfig() throws OkOPCException {
        try {
            List<DataValue> values = client().read(configNodeIdsRead).get();
            GasCO2.GasCO2Payload payload = new GasCO2.GasCO2Payload();
    ...
    @Override
    public void writeConfig(GasCO2Payload payload) throws OkOPCException {
        try {
    ...
    public static class GasCO2Payload {
        private Boolean isOn;
        private Float pv;
        private Float sv;
        private Float total;

        public GasCO2Payload() {
        }

        public GasCO2Payload(Boolean isOn, Float pv, Float sv, Float total) {
            this.isOn = isOn;
            this.pv = pv;
            this.sv = sv;
            this.total = total;
