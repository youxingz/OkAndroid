 ...
 */
@Deprecated
public class LeadShineJog {
    private ModbusMaster modbus;
    private int slaveId;

    public LeadShineJog(ModbusMaster master, int slaveId) {
        this.modbus = master;
     ...
     * 设置方向 0x1801
     *
     * @param direction {true} 正, {false} 反
     * @throws ModbusTransportException
     */
    public void direction(boolean direction) throws ModbusTransportException {
        WriteRegisterRequest request = new WriteRegisterRequest(slaveId, 0x1801, direction ? 0x4001 : 0x4002);
    ...
    public void velocity(int velocity) throws ModbusTransportException {
        WriteRegisterResponse response = (WriteRegisterResponse) modbus.send(new WriteRegisterRequest(slaveId, 0x01E1, velocity));
        if (response.isException()) {
            // error.
        }
    }
    ...
    public int velocity() throws ModbusTransportException {
        ReadHoldingRegistersResponse response = (ReadHoldingRegistersResponse) modbus.send(new ReadHoldingRegistersRequest(slaveId, 0x01E1, 2));
        if (response.isException()) {
            // error
            return -1;
        }
     ...
     * @throws ModbusTransportException
     */
    public void accelerationTime(int time) throws ModbusTransportException {
        WriteRegisterResponse response = (WriteRegisterResponse) modbus.send(new WriteRegisterRequest(slaveId, 0x01E7, time));
        if (response.isException()) {
            // error.
        }
    }
     ...
     * @throws ModbusTransportException
     */
    public int accelerationTime() throws ModbusTransportException {
        ReadHoldingRegistersResponse response = (ReadHoldingRegistersResponse) modbus.send(new ReadHoldingRegistersRequest(slaveId, 0x01E7, 2));
        if (response.isException()) {
            // error
            return -1;
        }
